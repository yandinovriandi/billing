...
const LinkSidebar = ({ href, children, active }) => {
    return <Link
        href={href}
        className={active ? 'flex items-center gap-3 bg-green-100 p-2 text-base font-semibold text-green-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700' :
...
export default function Sidebar() {
    return (
        <aside className="w-auto" aria-label="Sidebar">
            <div className="px-3 py-4 overflow-y-auto">
                <ul className="space-y-2">
                    <LinkSidebar href={route('dashboard')} active={route().current('dashboard') }>
                        <svg xmlns="http://www.w3.org/2000/svg" className="flex-shrink-0 w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
